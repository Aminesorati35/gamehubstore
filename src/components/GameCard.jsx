...
const GameCard = ({ game, onDownloadClick }) => {
  ...
  const baseClasses =
    "w-full font-semibold py-2.5 sm:py-3 px-3 sm:px-4 rounded-xl sm:rounded-2xl transition-all duration-300 flex items-center justify-center gap-1.5 sm:gap-2 cursor-pointer text-xs sm:text-sm border backdrop-blur-md active:scale-[0.98]";
  ...
  return (
    <div className="bg-white/[0.03] backdrop-blur-xl rounded-xl sm:rounded-2xl overflow-hidden border border-white/[0.08] shadow-[0_16px_40px_rgba(0,0,0,0.28)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.4)] hover:bg-white/[0.05] hover:border-white/20 transition-all duration-300 cursor-pointer hover:-translate-y-1.5 group">
      <div className="relative h-28 xs:h-32 sm:h-44 md:h-52 overflow-hidden">
        <img
          src={game.heroImage}
          alt={game.title}
      ...
      <div className="p-2.5 sm:p-4">
        ...
        {hasFeatures && (
          <div className="mb-2 sm:mb-3 flex flex-wrap gap-1 sm:gap-2 min-h-0 sm:h-17">
            {game.features.slice(0, 4).map((feature, index) => (
              <span
                key={index}
                className="px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md sm:rounded-lg text-[9px] sm:text-[11px] font-semibold bg-purple-500/15 text-purple-200 border border-purple-400/20"
              >
                {feature}
              </span>
            ))}
        ...
        {isRewards ? (
          <a href={game.downloadUrl} className={`${baseClasses} ${claimClasses}`}>
            ...
            <span className="hidden xs:inlin  e">Claim Rewards</span>
            <span className="xs:hidden">Claim Rewards</span>
          </a>
        ) : (
          <button onClick={handleDownload} className={`${baseClasses} ${downloadClasses}`}>
            ...
            <span className="xs:hidden">{isScript ? "Script" : "Download"}</span>
          </button>
        )}
      </div>
    </div>
